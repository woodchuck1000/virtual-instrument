...
def get_audios(audio_folder_path):
    ...
    for audio_path in audio_paths:
        ...
        if os.path.isfile(full_audio_path):  # 检查文件是否存在
            sound = pygame.mixer.Sound(full_audio_path)
            audios[audio_path] = sound  # 使用文件名作为键
        else:
            print(f"Audio file {full_audio_path} does not exist.")
    return audios
...
def start_playing(instrument):
    global cap, stream, audios, images, inf, yinfu_black, yinfu_red
    try:
        ...
        ret, _ = cap.read()
        if not ret:
            raise ValueError("无法从摄像头获取图像")
        inf = PatternHandler(instrument)
        audios = get_audios(f'{instrument}_audio')
        images,yinfu_black,yinfu_red = get_images(f'{instrument}_image')
...
def main_view():
    ...
    while True:
        ...
        if num >= 1:
            img.alpha_composite(images[num], dest=(-80, -20))
...
# 将图片作为背景
canvas.create_image(x1, y1, image=taodi_image, anchor="nw", tags="taodi")
canvas.create_image(x2, y1, image=hulusi_image, anchor="nw", tags="hulusi")
canvas.create_image(x3, y1, image=xun_image, anchor="nw", tags="xun")

# 创建按钮并添加到对应的列
btn_taodi = tk.Button(root, text="陶笛演奏", command=lambda: start_playing("taodi"))
btn_taodi.place(relx=0.1, rely=0.5, anchor="center")

btn_hulusi = tk.Button(root, text="葫芦丝演奏", command=lambda: start_playing("hulusi"))
btn_hulusi.place(relx=0.5, rely=0.5, anchor="center")

btn_xun = tk.Button(root, text="埙演奏", command=lambda: start_playing("xun"))
btn_xun.place(relx=0.9, rely=0.5, anchor="center")

# 设置按钮样式
style = ttk.Style()
style.configure("TButton", background="#0000FF")
style.configure("TButton", font=("宋体", 18))
