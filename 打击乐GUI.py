...
def draw_annotations(img, boxes, test_labels,yinjies):
    for box, test_label, yinjie in zip(boxes, test_labels, yinjies):
        ...
        if yinjie == 20:
            cv2.putText(img, str(test_label), (x1 + 10, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2,
                        cv2.LINE_AA)
            dot_center = (x1 + 20, y1 - 40)
            cv2.circle(img, dot_center, dot_radius, (0, 0, 255), -1)
        elif yinjie == 0:
            cv2.putText(img, str(test_label), (x1 + 10, y1 - 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2,
                        cv2.LINE_AA)
            dot_center = (x1 + 20, y1 - 10)
            cv2.circle(img, dot_center, dot_radius, (0, 0, 255), -1)
        else:
            cv2.putText(img, str(test_label), (x1 + 10, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2,
                        cv2.LINE_AA)
    return img
...
def start_playing():
    global cap, stream, audios, images, channel, num, now, hit
    try:
...
# 加载背景图片
background_image = Image.open("编钟GUI背景.png")
background_image = background_image.resize((1024, 768), Image.ANTIALIAS)
...
# 创建一个按钮，当点击时会启动演奏
start_button = tk.Button(frame, text="编钟演奏", command=lambda: threading.Thread(target=start_playing, daemon=True).start())
...
# 设置按钮样式
style = ttk.Style()
style.configure("TButton", background="#0000FF", font=("宋体", 14))  # 设置按钮背景颜色和字体（四号字体大约对应14号字体大小）
style.configure("TButton", padding=20)  # 设置按钮内边距
